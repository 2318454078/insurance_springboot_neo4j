...
public class ModelProcess {
	 ...
	 */
//	String rootDirPath = "/usr/local/HanLP/data";
	String rootDirPath = "/usr/local/HanLP/data";
	/**
	 * 分类模板索引
	 */
	...
	public  String queryAbstract(String querySentence) {
		...
		for (Term term : terms) {
			...
			if (termStr.contains("ic")) {        //nm 电影名
				abstractQuery += "ic ";
				abstractMap.put("ic", word);
				...
				nrCount++;
			}else if (termStr.contains("it")) {  //it 保险类型 x  评分
				abstractQuery += "it ";
				abstractMap.put("it", word);
			} else if (termStr.contains("in")) { //ng 类型
				abstractQuery += "in ";
				abstractMap.put("in", word);
			}
//            else if (termStr.contains("tit")) { //tit 类型
//                abstractQuery += "tit ";
//                abstractMap.put("tit", word);
//            }
			else {
				abstractQuery += word + " ";
			}
	...
	public  NaiveBayesModel loadClassifierModel() throws Exception {
		 ...
		 */
		SparkConf conf = new SparkConf().setAppName("NaiveBayesTest").setMaster("local[*]").set("spark.testing.memory","2147480000");
		 ...
		 * 英雄的评分是多少
		 */
		String scoreQuestions = loadFile("question/【0】投保年龄.txt");
		 ...
		 * 英雄什么时候上映的
		 */
		String timeQuestions = loadFile("question/【1】宽限期.txt");
		 ...
		 * 英雄是什么类型的
		 */
		String styleQuestions = loadFile("question/【2】犹豫期.txt");
		 ...
		 * 周星驰演了哪些电影
		 */
		String actorsMovie = loadFile("question/【3】受益人.txt");
		 ...
		 */
		String actorsQuestion = loadFile("question/【4】保险公司.txt");
		 ...
		 * 章子怡
		 */
		String actorInfos = loadFile("question/【5】保费缴纳方式.txt");
		 ...
		 * 周星驰出演的科幻类型的电影有哪些
		 */
		String genreMovies = loadFile("question/【6】保险期间.txt");
		 ...
		 * 英雄的简介、英雄的剧情是什么
		 */
		String storyQuestions = loadFile("question/【7】基本信息.txt");
...
//         * 周星驰演了哪些电影
//         */
//        String insuranceTerm = loadFile("question/【8】保险名词.txt");
//        sentences = insuranceTerm.split("`");
//        for (String sentence : sentences) {
//            double[] array = sentenceToArrays(sentence);
//            LabeledPoint train_one = new LabeledPoint(8.0, Vectors.dense(array));
//            train_list.add(train_one);
//        }
		String insuranceTerm = loadFile("question/【8】责任条款.txt");
		sentences = insuranceTerm.split("`");
		for (String sentence : sentences) {
			double[] array = sentenceToArrays(sentence);
			LabeledPoint train_one = new LabeledPoint(8.0, Vectors.dense(array));
